<?php
 ...
 session_start();
	...
	$first = mysqli_query($polaczenie2,"SELECT avg(ppm) AS ppm, DATE(mq7.date) AS date FROM mq7 WHERE DATE(mq7.date) = DATE_SUB(CURDATE(), INTERVAL 11 DAY)");
	$first1 = $first->fetch_assoc();
	$second = mysqli_query($polaczenie2,"SELECT avg(ppm) AS ppm, DATE(mq7.date) AS date FROM mq7 WHERE DATE(mq7.date) = DATE_SUB(CURDATE(), INTERVAL 12 DAY)");
	$second1 = $second->fetch_assoc();
	$third = mysqli_query($polaczenie2,"SELECT avg(ppm) AS ppm, DATE(mq7.date) AS date FROM mq7 WHERE DATE(mq7.date) = DATE_SUB(CURDATE(), INTERVAL 13 DAY)");
	$third1 = $third->fetch_assoc();
	$four = mysqli_query($polaczenie2,"SELECT avg(ppm) AS ppm, DATE(mq7.date) AS date FROM mq7 WHERE DATE(mq7.date) = DATE_SUB(CURDATE(), INTERVAL 14 DAY)");
	$four1 = $four->fetch_assoc();
	$five = mysqli_query($polaczenie2,"SELECT avg(ppm) AS ppm, DATE(mq7.date) AS date FROM mq7 WHERE DATE(mq7.date) = DATE_SUB(CURDATE(), INTERVAL 15 DAY)");
	$five1 = $five->fetch_assoc();
	$six = mysqli_query($polaczenie2,"SELECT avg(ppm) AS ppm, DATE(mq7.date) AS date FROM mq7 WHERE DATE(mq7.date) = DATE_SUB(CURDATE(), INTERVAL 16 DAY)");
	$six1 = $six->fetch_assoc();
	$seven = mysqli_query($polaczenie2,"SELECT avg(ppm) AS ppm, DATE(mq7.date) AS date FROM mq7 WHERE DATE(mq7.date) = DATE_SUB(CURDATE(), INTERVAL 17 DAY)");
